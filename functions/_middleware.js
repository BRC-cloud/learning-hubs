...
const SEC_HEADERS={'X-Frame-Options':'DENY','X-Content-Type-Options':'nosniff','Referrer-Policy':'strict-origin-when-cross-origin','Cache-Control':'no-cache, no-store, must-revalidate','Pragma':'no-cache'};

export async function onRequest(context){
  const cookie=context.request.headers.get('Cookie')||'';
  const authVal=getCookie(cookie,'_hub_auth');
  if(authVal && timingSafeEqual(authVal,TOKEN)){
    const resp=await context.next();
    const nr=new Response(resp.body,resp);
    for(const[k,v]of Object.entries(SEC_HEADERS))nr.headers.set(k,v);
    return nr;
  }
